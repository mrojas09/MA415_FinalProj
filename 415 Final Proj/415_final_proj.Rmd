...
# If dont have already make sure to have these packages installed, which I used to extract and read the data
# from twitter
...
# Data Description
For This project I used Twittes API and related packages to analyse word frequencies of a mass amount of Twitter feed, compareing them among different locations, later went on and tried to find some underlying relations of the data. I used 29 sample twitter dataset and each sample includes 5 minutes' twitter within an specific hour (from the last hour of 30 April to the 4th hour of 1 May (Est)). 
...
Here I grabbed 29 datasets and each contains 300 seconds grabbing of tweets. Then filtered are the region within United States "(-125, 25, -66, 50)". Continuously grabbing 24 hours of Twitter data will definitely probably burn out my computer, so my thought process is instead to be capturing a 300 seconds (5 minutes) sample for each hour of the day. 

# Data Cleaning

I merge all the subdatasets and add time_marks (for further adjustment of time-zone error, and geolocation analysis), clean the datasets, filter out sub-datasets for four states (MA, NY, FL, CA).
Then imported all the datasets into R, and marked them with different labels of their belonging hours.
```{r, warning=FALSE}
...
Then proceeded to add "time_mark" to each of the dataframes and keep only needed variables.
...
Next, filter and remove tweets with no geo_enabled and not English, and filter tweets with extreme locations. After that, prepare the five-state sub-datasets, for further "popularity" analysis:
...
First of all, I want to generate an interactive distribution plot of all twitters using ShinyApp to show the trend of all twitter's distribution within the continent of United States through timeline in one day (and a little more at both tails). After that also plot the Twitter activity in MA, NY, FL and CA respectively.
...
Here is the plot of the number of tweets posted in different time in one day for the states chosen. It is obvious that from 3am to 5am the number of tweets is lowest and from 21pm to 23pm the number is highest. CA has the highest number of tweets, since its population is large, and the number goes up sharply in the morning. Whereas the curves for FL, and MA have ther own similar pattern.

The following will help us have a sligthly better understanding of this. Let us take a look at the number of tweets in different cities.
```{r}
...
In the above I decided to plot the TOP 10 dense cities found. Not surprisingly, in MA and FL most tweets are posted in Boston. In CA and NY, more cities have large number of tweets. Apart from Los Angeles and Manhattan, San Francisco, San Diego, San Jose, Brooklyn, Queens, Bronx aslo have large number of tweets.
...
Now combining all cities , here we can see what the TOP 15 cities for highest tweet density are.
...
I dont think it is fair to compare two cities if their population are greatly different. Thus I define density here by the number of tweets per person in this state and make a plot of tweet density for these states. Now DC stands out, having the largest density at any time in one day, and MA, FL have the same pattern for density this time.

But do they truly have the same density pattern? First I want to test whether they have the same density variance in one day and i will be doing so in the following:
...
I make pair plots of these variables and calculate the correlation amony them. From the output, followers count and listed count have strong linear relation and friends count also have slight linear relation with these two variables.So first make a simple linear regression between followers count and listed count.
...
Now we are able to see the most frequent words in the sample Twitter data. Build word cloud
